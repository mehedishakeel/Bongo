...
void Layout::loadDesc() {
  ...
  // Present bundled compatibility layouts as part of Bongo. Original authorship
  // and licenses remain documented in the repository and source headers.
  const QString builtInRoot = QFileInfo(LayoutsFilePath()).absoluteFilePath() + "/";
  if (QFileInfo(fin.fileName()).absoluteFilePath().startsWith(builtInRoot)) {
    if (lD.name == "Avro Phonetic") {
      lD.name = "Bongo Phonetic";
    } else if (lD.name == "Avro Easy") {
      lD.name = "Bongo Easy";
    }
    lD.devName = "Bongo Project";
    lD.devComment = "A built-in Bangla keyboard layout included with Bongo.";
  }
}
