...
void TopBar::SetupTopBar() {
  ...
  // Keep the unobtrusive toolbar near the system status area on every launch.
  if (QScreen *screen = QGuiApplication::primaryScreen()) {
    const QRect available = screen->availableGeometry();
    const int margin = 16;
    move(available.right() - width() - margin + 1, available.top() + margin);
  }
}
...
void TopBar::checkForUpdate(bool notifyIfCurrent) {
  ...
  const QUrl url(QString("https://api.github.com/repos/%1/releases?per_page=1").arg(GITHUB_REPOSITORY));
  ...
  connect(updateReply, &QNetworkReply::finished, this, [this]() {
    ...
    if (statusCode == 404) {
      if (reportCurrent) {
        QMessageBox::information(this, "Bongo", QString("Bongo %1 is installed. No public release information is available yet.").arg(qApp->applicationVersion()));
      }
      return;
    }
    ...
    if (failed || parseError.error != QJsonParseError::NoError || !document.isArray()) {
      if (reportCurrent) {
        QMessageBox::warning(this, "Bongo", "Couldn’t check for updates. Check your internet connection and try again.");
      }
      return;
    }
    ...
    if (releases.isEmpty()) {
      if (reportCurrent) {
        QMessageBox::information(this, "Bongo", QString("Bongo %1 is up to date. No newer release is published yet.").arg(qApp->applicationVersion()));
      }
      return;
    }
    ...
    if (tag.isEmpty() || !releaseUrl.isValid()) {
      if (reportCurrent) {
        QMessageBox::warning(this, "Bongo", "The newest release information could not be read. Open GitHub Releases and check manually.");
      }
      return;
    }
...
void TopBar::layoutMenuInstall_clicked() {
  QString fileName = QFileDialog::getOpenFileName(Q_NULLPTR, "Select Keyboard Layout", QDir::homePath(),
                                                  "Bongo-compatible Keyboard Layout (*.avrolayout)");
  ...
  if (fileName.contains(".avrolayout") && fileName != "") {
    ...
    case UnsupportedLayout:
      QMessageBox::critical(Q_NULLPTR,
                            "Bongo",
                            "Unsupported layout file.\nBongo supports compatible .avrolayout files.",
                            QMessageBox::Ok);
      break;
    case OpenError:
