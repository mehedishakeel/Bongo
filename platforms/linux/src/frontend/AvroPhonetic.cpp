...
AvroPhonetic::AvroPhonetic() {
  ...
  if (!layoutFile.open(QIODevice::ReadOnly)) {
    LOG_ERROR("[AvroPhonetic]: Error: Couldn't open layout file!\n");
  }
...
AvroPhonetic::~AvroPhonetic() = default;

QString AvroPhonetic::convert(QString input) {
...
QString AvroPhonetic::fixString(QString input) {
  QString fixed;
...
bool AvroPhonetic::isVowel(QChar c) {
  return vowel.contains(c, Qt::CaseInsensitive);
}

bool AvroPhonetic::isConsonant(QChar c) {
  return cons.contains(c, Qt::CaseInsensitive);
}

bool AvroPhonetic::isPunctuation(QChar c) {
  return (!(isVowel(c) || isConsonant(c)));
}

bool AvroPhonetic::isNumber(QChar c) {
  return num.contains(c, Qt::CaseInsensitive);
}

bool AvroPhonetic::isExact(QString needle, QString heystack, int start, int end, bool strnot) {
  int len = end - start;
  return ((start >= 0 && end < heystack.length() && (heystack.mid(start, len) == needle)) ^ strnot);
}

bool AvroPhonetic::isCaseSensitive(QChar c) {
