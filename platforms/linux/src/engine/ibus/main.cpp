...
void start_setup(bool ibus) {
  ...
  if (ibus) {
    ibus_bus_request_name(bus, "org.freedesktop.IBus.Bongo", 0);
  } else {
    component = ibus_component_new("org.freedesktop.IBus.Bongo",
                                   "Bongo",
                                   PROJECT_VERSION,
                                   "GPL 3",
                                   "Mehedi Shakeel",
                                   "",
                                   PROJECT_DATADIR "/ibus-bongo --ibus",
                                   "bongo"
    );

    ibus_component_add_engine(component,
                              ibus_engine_desc_new("Bongo",
                                                   "Bongo",
                                                   "Bongo IME for iBus",
                                                   "bn",
                                                   "GPL 3",
                                                   "Mehedi Shakeel",
                                                   PROJECT_DATADIR "/icons/Bongo.png",
                                                   "default"
                              ));
    ibus_bus_register_component(bus, component);

    ibus_bus_set_global_engine_async(bus, "Bongo", -1, nullptr, nullptr, nullptr);
  }
  ibus_main();
}
