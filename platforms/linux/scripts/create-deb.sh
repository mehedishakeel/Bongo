...
if [ -n "${BONGO_GITHUB_REPOSITORY:-}" ]; then
  UPDATE_ARGS+=("-DBONGO_UPDATE_URL=https://github.com/${BONGO_GITHUB_REPOSITORY}/releases/latest/download/linux.json")
  UPDATE_ARGS+=("-DBONGO_GITHUB_REPOSITORY=${BONGO_GITHUB_REPOSITORY}")
fi
