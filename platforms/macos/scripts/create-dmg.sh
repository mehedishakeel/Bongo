...
mkdir -p "$BONGO_ROOT/dist"
STAGE=$(mktemp -d "$BONGO_ROOT/dist/.stage.XXXXXX")
trap 'rm -rf "$STAGE"' EXIT
ditto "$APP_BUNDLE" "$STAGE/Bongo.app"
cp "$BONGO_ROOT/docs/MACOS-INSTALL.md" "$STAGE/README.md"
cp "$BONGO_ROOT/THIRD_PARTY_NOTICES.md" "$STAGE/"
