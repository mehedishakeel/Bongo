...
enum BongoFontSettings {
    ...
    static func font() -> NSFont {
        if let familyName, let chosen = NSFont(name: familyName, size: size) { return chosen }
...
class CandidateView: NSView {
    ...
    private var rowHeight: CGFloat { max(24, BongoFontSettings.size + 10) }
