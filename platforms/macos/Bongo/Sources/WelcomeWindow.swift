...
class WelcomeTabView: NSView {
    ...
    override init(frame: NSRect) {
        ...
        let symbols = ["sparkles", "keyboard", "slider.horizontal.3", "textformat"]
        ...
        NSLayoutConstraint.activate([
            ...
            sidebar.bottomAnchor.constraint(equalTo: bottomAnchor), sidebar.widthAnchor.constraint(equalToConstant: 198),
            brandIcon.leadingAnchor.constraint(equalTo: sidebar.leadingAnchor, constant: 18), brandIcon.topAnchor.constraint(equalTo: sidebar.topAnchor, constant: 20),
            brandIcon.widthAnchor.constraint(equalToConstant: 34), brandIcon.heightAnchor.constraint(equalToConstant: 34),
            ...
            nav.leadingAnchor.constraint(equalTo: sidebar.leadingAnchor, constant: 16), nav.trailingAnchor.constraint(equalTo: sidebar.trailingAnchor, constant: -16),
            ...
            heading.leadingAnchor.constraint(equalTo: sidebar.trailingAnchor, constant: 28), heading.topAnchor.constraint(equalTo: topAnchor, constant: 22),
            ...
            footer.centerXAnchor.constraint(equalTo: tabView.centerXAnchor), footer.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -14),
        ])
        refreshNavigation(0)
    }
    required init?(coder: NSCoder) { fatalError() }
...
/// Candidate-popup typography. Apps receiving Bangla text still choose their
/// own document font; these settings affect only Bongo's suggestion panel.
final class FontsView: NSView {
    private let fontMenu = NSPopUpButton()
    private let sizeMenu = NSPopUpButton()
    private let preview = NSTextField(wrappingLabelWithString: "আমি বাংলায় লিখি  ·  বাংলা আমার ভাষা")
    ...
    private func setupUI() {
        ...
        let intro = NSTextField(wrappingLabelWithString:
            "Choose the Bangla font and text size used in Bongo's suggestion popup. Text inside other apps keeps that app's own font.")
        ...
        let fontColumn = NSStackView(views: [fontLabel, fontMenu])
        fontColumn.orientation = .vertical
        fontColumn.alignment = .leading
        fontColumn.spacing = 6
        let sizeColumn = NSStackView(views: [sizeLabel, sizeMenu])
        sizeColumn.orientation = .vertical
        sizeColumn.alignment = .leading
        sizeColumn.spacing = 6
        let controls = NSStackView(views: [fontColumn, sizeColumn])
        controls.orientation = .horizontal
        controls.alignment = .bottom
        controls.spacing = 16
        fontMenu.widthAnchor.constraint(equalToConstant: 270).isActive = true
        sizeMenu.widthAnchor.constraint(equalToConstant: 145).isActive = true

        let settings = NSStackView(views: [controls])
        settings.orientation = .vertical
        settings.alignment = .leading
        let settingsCard = CardContainer(
            content: settings,
            insets: NSEdgeInsets(top: 18, left: 20, bottom: 18, right: 20))

        let previewHeader = WelcomeUI.sectionHeader("Preview")
        preview.alignment = .center
        preview.maximumNumberOfLines = 2
        preview.translatesAutoresizingMaskIntoConstraints = false
        preview.heightAnchor.constraint(greaterThanOrEqualToConstant: 90).isActive = true
        let previewCard = CardContainer(
            content: preview,
            insets: NSEdgeInsets(top: 18, left: 20, bottom: 18, right: 20))

        let fontsHeader = WelcomeUI.sectionHeader("Install more fonts")
        let infoTitle = NSTextField(labelWithString: "Use any Bangla font installed on this Mac")
        infoTitle.font = .systemFont(ofSize: 14, weight: .semibold)
        let infoBody = NSTextField(wrappingLabelWithString:
            "Install a font with Font Book, then return here and choose Refresh Fonts. Bongo lists fonts that can draw Bangla text.")
        infoBody.font = .systemFont(ofSize: 12)
        infoBody.textColor = .secondaryLabelColor
        let refresh = NSButton(title: "Refresh Fonts", target: self, action: #selector(refreshFonts))
        refresh.bezelStyle = .rounded
        let openBook = NSButton(title: "Open Font Book", target: self, action: #selector(openFontBook))
        openBook.bezelStyle = .rounded
        let actions = NSStackView(views: [refresh, openBook])
        actions.orientation = .horizontal
        actions.spacing = 8
        let info = NSStackView(views: [infoTitle, infoBody, actions])
        info.orientation = .vertical
        info.alignment = .leading
        info.spacing = 6
        info.setCustomSpacing(12, after: infoBody)
        let infoCard = CardContainer(content: info)

        let content = NSStackView(views: [header, intro, settingsCard, previewHeader, previewCard, fontsHeader, infoCard])
        content.orientation = .vertical
        content.alignment = .leading
        content.spacing = 8
        content.setCustomSpacing(18, after: intro)
        content.setCustomSpacing(26, after: settingsCard)
        content.setCustomSpacing(10, after: previewHeader)
        content.setCustomSpacing(26, after: previewCard)
        content.setCustomSpacing(10, after: fontsHeader)
        ...
        NSLayoutConstraint.activate([
            ...
            settingsCard.widthAnchor.constraint(equalTo: content.widthAnchor),
            previewCard.widthAnchor.constraint(equalTo: content.widthAnchor),
            infoCard.widthAnchor.constraint(equalTo: content.widthAnchor),
        ])
        updatePreview()
    }
    ...
    private func configureSizeMenu() {
        let sizes: [CGFloat] = [14, 16, 18, 20, 22, 24]
        sizeMenu.addItems(withTitles: sizes.map { "\(Int($0)) pt" })
        sizeMenu.target = self
        sizeMenu.action = #selector(sizeChanged)
        let current = BongoFontSettings.size
        let chosen = sizes.enumerated().min(by: { abs($0.element - current) < abs($1.element - current) })?.offset ?? 1
        sizeMenu.selectItem(at: chosen)
    }

    private func reloadFonts() {
        let selected = BongoFontSettings.familyName
        fontNames = NSFontManager.shared.availableFontFamilies
            .filter(supportsBangla)
            .sorted { $0.localizedStandardCompare($1) == .orderedAscending }
        fontMenu.removeAllItems()
        fontMenu.addItem(withTitle: "System default")
        fontMenu.addItems(withTitles: fontNames)
        if let selected, let index = fontNames.firstIndex(of: selected) {
            fontMenu.selectItem(at: index + 1)
        } else {
            fontMenu.selectItem(at: 0)
        }
    }

    private func supportsBangla(_ family: String) -> Bool {
        let font = CTFontCreateWithName(family as CFString, 16, nil)
        var characters = Array("বাংলা".utf16)
        var glyphs = Array(repeating: CGGlyph(), count: characters.count)
        return characters.withUnsafeMutableBufferPointer { chars in
            glyphs.withUnsafeMutableBufferPointer { output in
                CTFontGetGlyphsForCharacters(font, chars.baseAddress!, output.baseAddress!, chars.count)
            }
        }
    }

    @objc private func fontChanged() {
        BongoFontSettings.familyName = fontMenu.indexOfSelectedItem == 0 ? nil : fontMenu.titleOfSelectedItem
        applyChanges()
    }

    @objc private func sizeChanged() {
        BongoFontSettings.size = CGFloat([14, 16, 18, 20, 22, 24][max(0, sizeMenu.indexOfSelectedItem)])
        applyChanges()
    }
    ...
    @objc private func openFontBook() {
        NSWorkspace.shared.openApplication(
            at: URL(fileURLWithPath: "/System/Applications/Font Book.app"),
    ...
    private func updatePreview() {
        preview.font = BongoFontSettings.font()
    }
}
...
class SettingsView: NSView {
    ...
    private func setupUI() {
        ...
        content.alignment = .leading
        content.spacing = 6
        content.setCustomSpacing(12, after: intro)
        content.setCustomSpacing(16, after: cardStack)
        content.setCustomSpacing(10, after: suggestionsHeader)
        content.setCustomSpacing(14, after: emojiCard)
    ...
    private func makeEmojiCard() -> NSView {
        ...
        return CardContainer(
            content: row,
            insets: NSEdgeInsets(top: 10, left: 16, bottom: 10, right: 16))
    }
...
class GettingStartedView: NSView {
    override init(frame: NSRect) {
        ...
        let hero = RoundedTintView(cornerRadius: 16, fill: { WelcomeUI.accent.withAlphaComponent(0.10) })
        let eyebrow = label("MADE FOR YOUR WORDS", size: 10, weight: .semibold, color: WelcomeUI.accent)
        let headline = label("নিজের ভাষায়, নিজের মতো।", size: 28, weight: .medium)
        let caption = label("Think in Bangla. Type naturally.", size: 14, color: .secondaryLabelColor)
        let example = label("ami banglay likhi   →   আমি বাংলায় লিখি", size: 16, weight: .medium)
        ...
        heroStack.spacing = 8
        heroStack.setCustomSpacing(14, after: caption)
        ...
        NSLayoutConstraint.activate([
            ...
            heroStack.topAnchor.constraint(equalTo: hero.topAnchor, constant: 18),
            heroStack.bottomAnchor.constraint(equalTo: hero.bottomAnchor, constant: -18),
        ])
        ...
        setup.spacing = 7
        let card = CardContainer(content: setup, insets: NSEdgeInsets(top: 13, left: 18, bottom: 13, right: 18))
