...
Begin
		 ...
		 Mutex := CreateMutex(Nil, True, 'Bongo_Keyboard');
		 If (Mutex = 0) Or (GetLastError <> 0) Then Begin

					uCommandLineFunctions.SendCommand('restore');

					Application.MessageBox('Bongo is already running on this system and' + #10 + 'running more than one instance is not allowed.', 'Bongo',
		 ...
		 Application.CreateForm(TBongoMainForm1, AvroMainForm1);
