...
Begin
		 ...
		 If (KC_Key = '1') Or (KC_Key = '2') Or (KC_Key = '3') Or (KC_Key = '4') Or (KC_Key = '5') Or (KC_Key = '6') Or (KC_Key = '7') Or (KC_Key = '8') Or (KC_Key = '9') Or (KC_Key = '0') Or
					...
					Try
							 ...
							 For I := 0 To Node.NodeCount - 1 Do Begin
										If Node.Nodes[I].NodeCount <= 0 Then
												 ...
												 KeyData.AddOrSetValue(String(Node.Nodes[I].Name), '')
										Else
												 // if item has cdata
												 KeyData.AddOrSetValue(String(Node.Nodes[I].Name), Node.Nodes[I].Nodes[0].ValueAsUnicodeString);
										Application.ProcessMessages;
							 End;
		 ...
		 Try
					Try
							 If LowerCase(LayoutPath) <> 'avrophonetic*' Then Begin
										...
										InternalName := XML.Root.FindNode('LayoutName').Nodes[0].ValueAsUnicodeString;
										Application.ProcessMessages;
										Developer := XML.Root.FindNode('DeveloperName').Nodes[0].ValueAsUnicodeString;
										Application.ProcessMessages;
										Version := XML.Root.FindNode('LayoutVersion').Nodes[0].ValueAsUnicodeString;
										Application.ProcessMessages;
										Comment := XML.Root.FindNode('DeveloperComment').Nodes[0].ValueAsUnicodeString;
										Application.ProcessMessages;
							 End
							 Else Begin
		 ...
		 Try
					Try
							 ...
							 If FileExists(GetAvroDataDir + 'Keyboard Layouts\' + FileName) Then Begin
										If Application.MessageBox(pchar('Keyboard layout "' + FileName + '" is already installed.' + #10 + 'Do you want to overwrite it?'), 'Bongo',
							 ...
							 If MyCopyFile(LayoutPath, GetAvroDataDir + 'Keyboard Layouts\' + FileName, Overwrite) Then Begin
										...
										Application.MessageBox
											(pchar('Keyboard layout "' + FileName + '" has been installed successfully!' + #10 + '' + #10 + 'Layout name: ' + InternalName + #10 + 'Version: ' + Version + #10 +
																'Developer: ' + Developer + #10 + 'Comment: ' + Comment + #10), 'Bongo', MB_OK + MB_ICONASTERISK + MB_DEFBUTTON1 + MB_SYSTEMMODAL);
							 ...
							 Else Begin
										Result := False;
										Application.MessageBox(pchar('Keyboard layout "' + FileName + '" was not installed!'), 'Bongo', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 + MB_SYSTEMMODAL);
							 End;
					...
					Except
							 On E: Exception Do Begin
										...
										Application.MessageBox(pchar('Error installing keyboard layout!' + #10 + '' + #10 + '' + #10 + 'Make sure this is a valid keyboard layout file' + #10 + 'or,' + #10 +
																'You have enough permission to write in keyboard layouts folder' + #10 + 'or,' + #10 + 'Keyboard layouts directory is writable'), 'Bongo',
