     ...
     Try
          ...
          Except
               On E: Exception Do Begin
                    Application.MessageBox(PChar('Error loading skin!' + #10 + '' + #10 + 'Make sure ' + ExtractFileName(FullSkinPath) + ' is a valid skin file or skin is not corrupt.'), 'Bongo', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 + MB_APPLMODAL);
                    Result := False;
               End;
          End;
     Finally
...
Begin
     Try
          ...
          Except
               On E: Exception Do Begin
                    Application.MessageBox(Pchar('Error loading skin preview image!' + #10 + '' + #10 + 'Make sure ' + ExtractFileName(SkinPath) + ' is a valid skin file or skin is not corrupt.'), 'Bongo', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 + MB_APPLMODAL);
               End;
          End;
     Finally
          FreeAndNil(XML);
...
Begin
     ...
     Try
          ...
          Except
               On E: Exception Do Begin
                    Application.MessageBox(PChar('Error loading skin description!' + #10 + '' + #10 + 'Make sure ' + ExtractFileName(SkinPath) + ' is a valid skin file or skin is not corrupt.'), 'Bongo', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 + MB_APPLMODAL);
                    result := False;
               End;
          End;
     Finally
     ...
     Try
          Try
               ...
               If FileExists(GetAvroDataDir + 'Skin\' + Filename) Then Begin
                    If Application.MessageBox(pchar('Skin "' + Filename + '" is already installed.' + #10 + 'Do you want to overwrite it?'), 'Bongo', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2 + MB_SYSTEMMODAL) = ID_YES Then
                         Overwrite := True
                    Else
                         Overwrite := False;
               End;
               ...
               If MyCopyFile(SkinPath, GetAvroDataDir + 'Skin\' + Filename, Overwrite) Then Begin
                    ...
                    Application.MessageBox(Pchar('Skin "' + Filename + '" has been installed successfully!' + #10 + '' + #10 +
                         ...
                         '' + #10 + 'Please use the settings dialog box to activate this skin.'), 'Bongo', MB_OK + MB_ICONASTERISK + MB_DEFBUTTON1 + MB_SYSTEMMODAL);
               ...
               Else Begin
                    ...
                    Application.MessageBox(pchar('Skin "' + Filename + '" was not installed!'),
                         'Bongo', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 + MB_SYSTEMMODAL);
               End;
          Except
               On e: exception Do Begin
                    result := False;
