					...
					Try
							 ...
							 Except
										On E: Exception Do Begin
												 Application.MessageBox(PChar('Cannot import auto-correct dictionary!'), 'Bongo', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 + MB_APPLMODAL);
												 result := False;
										End;
							 End;
					Finally
...
Begin
		 If OpenDialog1.Execute(Self.Handle) Then Begin
					...
					For S in idict.Keys Do Begin

							 Rs := S;
							 Ws := idict.Items[S];

							 If dict.TryGetValue(Rs, DictItem) Then Begin
										// There may be a conflict
										If DictItem <> Ws Then Begin
		 ...
		 Except
					On E: Exception Do Begin
							 Application.MessageBox
								 (PChar('Cannot save to dictionary!' + #10 + '' + #10 + '-> Make sure the disk is not write protected, or' + #10 + '-> ' + Path + ' file is not ''Read Only'', or' + #10 +
													 '-> You have necessary account privilege to modify content.' + #10 + '' + #10 + 'Please ask your System Administrator or contact' + #10 +
													 'OmicronLab (http://www.omicronlab.com/forum/) to solve of this problem.'), 'Auto correct', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 + MB_APPLMODAL);
							 result := False;
					End;
		 End;
