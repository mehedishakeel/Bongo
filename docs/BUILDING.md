...
The output is `dist/Bongo-1.0-macos-arm64.dmg`. The script applies an ad-hoc
signature. Public distribution should use an Apple Developer ID signature and
notarization.
...
The output is `dist/windows/Bongo.exe`. A successful compile alone is not a
